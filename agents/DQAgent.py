...
class DQAgent(RLAgent):
  ...
  def learn(self, render_env, MAX_EPISODE_LENGTH = float("inf")):
      timestep = 0
      ...
      writer = tf.train.SummaryWriter(self.summary_dir, self.sess.graph)
      ...
      while timestep < self.max_training_steps:
        ...
        if terminal or (episode_length+1)%MAX_EPISODE_LENGTH == 0:
  ...
  def test(self, max_episodes, render_env, MAX_EPISODE_LENGTH = float("inf")):
      timestep = 0
      curr_state = self.env.reset() #Get the initial state
